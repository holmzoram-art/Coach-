# Planen

Dette er hele planen. Les den én gang. Du skal ikke måtte slå opp i noe.

`matplan.md` og `makrogrunnlag.md` er referanse hvis du vil ha variasjon eller
detaljer. **Du trenger dem ikke.**

---

## Hver dag — det samme, uansett

| Kl. | Hva |
|---|---|
| **07** | Beger skyr eller yoghurt + **1 dl havregryn (tørt, rett i begeret)** + neve frosne bær + **1 spiseskje nøtter** — *eller 3 cottage cheese-rundstykker med kokt skinke* |
| **11** | **Kantina:** halv tallerken salat først · kvart tallerken protein · kvart tallerken karbo |
| **14** | **Hele begeret cottage cheese** + en frukt. Ikke en «porsjon» fra pakken — hele |
| **Middag** | Protein på størrelse med håndflaten din (**mål det rått**) · **dobbel** grønnsak · **halv pose ferdigris eller 3 poteter** |
| **21** | Beger cottage cheese + bær. Varm bærene i mikroen og hell over — ikke varm osten |

Fem måltider. De er like hver dag. **Du velger ingenting.**

Nøttene kl. 07 er ikke valgfrie — de er det eneste fettet i planen.

---

## Løper du den dagen — to ting skjer i tillegg

| | |
|---|---|
| **15:30, fra skuffen på jobb** | 1 Corny Big + 1 neve rosiner |
| **Rett etter økta** | 1 YT Protein restitusjonsdrikk |

Det er alt. Ingen andre justeringer på en vanlig løpedag.

---

## Den ene regelen du må huske

> **Løper du over 10 km: ta full karboporsjon i kantina, og dobbel karbo til
> middag — hel pose ris i stedet for halv, eller 6 poteter i stedet for 3.**

Én regel. Den henger på et tall du ser på klokka.

### Porsjonene, uten tvil

| | Vanlig dag | Over 10 km |
|---|---|---|
| **Ferdigris i pose** (allerede kokt) | **Halv pose** | **Hel pose** |
| **Ris du koker selv** | **50 g tørr** = en tiendedel av en 1 kg-pose | **100 g tørr** = en femtedel |
| **Pasta** | **60 g tørr** = en åttedel av en 500 g-pakke | **125 g tørr** = **en kvart pakke** |
| **Poteter** | **3 middels** | **6 middels** |
| **Gryta fra søndag** | 1 dyp tallerken | + poteter ved siden av |

**Hvorfor gram akkurat her:** pasta og ris er de eneste varene der volum
lyver. Penne og spagetti fyller samme desiliter med helt ulik mengde. Alt
annet i planen er utformet slik at det ikke kan misforstås.

**Du trenger ikke kjøkkenvekt.** Alle tallene er brøkdeler av en pakke:
en kvart 500 g-pakke pasta, en tiendedel av en riskilo. Har du vekt, bruk den
gjerne — men det er ikke forutsetningen.

### De tre tørrvarene, i gram

| | Mål | ≈ gram |
|---|---|---|
| **Havregryn** | 1 dl, tørt, rett i begeret | **35 g** |
| **Nøtter** | 1 spiseskje | **15 g** |
| **Rosiner** | 1 neve | **40 g** |

Havregryn og rosiner er ett produkt hver, så desiliteren og neven er trygge
der — gramtallet står bare hvis du vil ha det.

### Resten måles som før

**Håndflate** (protein, målt rått) · **beger** (yoghurt, kvarg, cottage
cheese) · **neve** (bær) · **pose** (ferdigris) · **dobbel grønnsak**.
Ingen av dem skal veies. De er entydige som de er.

**Alle porsjoner er ferdig tilberedt, bortsett fra tre:**
havregryn, pasta og ris måles tørt, og proteinet måles rått.

---

## Fredag — langtur

Fredag er den eneste dagen som er annerledes, og den er annerledes fordi
magen din ennå ikke er avklart.

| Kl. | Hva |
|---|---|
| **07** | Frokost som vanlig, **men uten nøtter** |
| **09:30** | 1 Corny Big + neve rosiner (skuffen) |
| **11** | Kantina, **full karbo**. Ingenting fett — ikke lasagne, ikke panert, ikke fløtesaus |
| **13:30** | 1 stort glass saft + 1 Corny Big + 3 riskaker (skuffen) |
| **15:15** | Neve rosiner |
| **Under** | Følg det som står i `mage.md`. Ikke improviser |
| **Etter** | YT Protein restitusjonsdrikk, så middag som vanlig |

**Fredagsregelen i én setning: ingenting fett etter frokost, og spis av
skuffen tre ganger.**

---

## To rutiner i uka. Ikke fem.

### Lørdag — handle
Samme liste hver uke. Se nederst.

### Søndag — 45 minutter på kjøkkenet, så pakk posen

**Tre ting, det meste passivt:**
1. **Ovnen på 200°: 3 kg poteter i båter.** 35 min, du gjør noe annet
2. **Én stor gryte eller suppe, 4 porsjoner.** Kjøttdeig eller linser,
   hakkede tomater, frossen grønnsaksblanding, løk
3. **Kok 10 egg.** La skallet være på

**Så pakker du jobbposen** og setter den i gangen: 5 begre cottage cheese. Inn i kjøleskapet på jobb mandag morgen.

**Det som er laget søndag spises innen torsdag.** Torsdag kveld: fryses eller
kastes. Det er hele holdbarhetsregelen.

---

## Én gang i måneden — bak rundstykker

**Én ovnsomgang, to brett, 24 rundstykker. 45 minutter, hvorav 30 er ovntid.**

| Ingrediens | Mengde |
|---|---|
| Cottage cheese | **1 kg** (2,5 begre) |
| Egg | **12** |
| Havregryn | **375 g** |
| Bakepulver | 2 ts |
| Salt | 1 ts |

Rør alt sammen, la det svelle 10 min, klatt ut 12 per brett. **200 °C, 30 min.**

Vil du ha forsyning til en hel måned: **rør dobbelt og stek i to omganger.**
Totalt ~1 t 20 min, det meste venting. 48 stk tar en hel fryserhylle — sjekk
at du har plass før du gjør det.

**Frys dem 3 og 3 i små poser.** Da tar du én pose om morgenen og slipper å
telle. Avkjøl helt før frysing, ellers blir de seige.

**Tining:** 40–60 sekunder i mikro fra frossen, eller ta posen ut kl. 07 så er
de tint til lunsj. De tåler å ligge i veska hele dagen — men **legg på skinke
først når du skal spise**, eller sett dem i kjøleskapet på jobb.

**Hva de dekker:**

| Luke | Antall | Dom |
|---|---|---|
| **Frokost** | 3 stk + kokt skinke | **Ja.** ~400 kcal, 32 g protein |
| **Kl. 14 på jobb** | 2 stk + skinke | **Ja.** Bedre enn en bar |
| **Kl. 21** | 2 stk | Går an, men cottage cheese med bær metter bedre |
| **15:30 før økt** | — | **Nei.** For lite karbo, for mye fett. Skuffen står |
| **Erstatte kantinelunsjen** | 4 stk | **Bare når du må.** Kantina gir deg en halv tallerken grønnsaker som rundstykkene ikke har |

---

## Skuffen på jobb

Fylles **når den er tom**, ikke etter en plan. Står på handlelista permanent.

- **Corny Big** — 3 pakker om gangen, varer en måned
- **Rosiner** i en tett boks — tørkede tranebær funker like bra, men koster mer
- **Riskaker** og en flaske **saft**
- **3 proteinbarer** — nødreserve for dagen du glemte posen

---

## Middagene — fire, gå i ring

Alle bygges likt: **protein (håndflate, målt rått) · dobbel grønnsak · halv pose ris eller 3 poteter.**

1. **Gryta fra søndag** (varmes, 8 min) — to ganger i uka
2. **Kylling eller svin + potetene fra søndag + grønnsaker** (15 min)
3. **Fisk fra frossen + potetene + grønnsaker** (18 min)
4. **Wok: kjøtt eller kylling + frossen wokblanding + ferdigris fra pose** (15 min)

Ingen ukedag over 18 minutter. Lørdag lager du hva du vil.

---

## Handlelista — nesten lik hver uke

**Fast:**
egg 2 brett · **cottage cheese 8 begre** · skyr eller yoghurt 5 begre · frosne bær
1 pose · nøtter 1 liten pose · poteter 3 kg · frossen grønnsaksblanding 2 poser
· ferdigris 4 poser · hakkede tomater 2 bokser · løk · kjøtt eller fisk til
4 middager · kokt skinke · bananer

**Når det er tomt:**
havregryn · Corny Big 3 pk · rosiner · riskaker · saft · proteinbar · frossen fisk

Det er hele lista. Den ser lik ut neste uke.

---

## Én dato i kalenderen

**28. september:** fra da tar du **liten** karboporsjon i kantina på vanlige
dager. Full porsjon bare på dager du løper over 10 km, og på fredag.

Det er den eneste endringen mellom nå og januar.

---

## Hvis alt annet ryker

**14-måltidet.** Det er det som avgjør kvelden din. Alt annet kan glippe en dag.
